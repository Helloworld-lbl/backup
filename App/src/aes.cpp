...
// AES加密函数
int AES::encrypt(const std::string &file, const std::string &targetFile, const std::string &key)
{
    // 读取输入文件
    ...
    EVP_EncryptInit_ex(ctx, EVP_aes_128_ecb(), nullptr, hashed_key, nullptr);

    // 加密数据
    unsigned char in_buffer[AES_BLOCK_SIZE];
    unsigned char out_buffer[AES_BLOCK_SIZE];
    int len;
    while (infile.read(reinterpret_cast<char *>(in_buffer), AES_BLOCK_SIZE))
    {
        EVP_EncryptUpdate(ctx, out_buffer, &len, in_buffer, AES_BLOCK_SIZE);
...
// AES解密函数
int AES::decrypt(const std::string &file, const std::string &targetFile, const std::string &key)
{
    // 读取输入文件
    std::ifstream infile(file, std::ios::binary);
    if (!infile.is_open())
    ...
    // 创建并初始化解密上下文
    EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
    EVP_DecryptInit_ex(ctx, EVP_aes_128_ecb(), nullptr, hashed_key, nullptr);
    ...
    while (infile.read(reinterpret_cast<char *>(in_buffer), AES_BLOCK_SIZE))
    {
        EVP_DecryptUpdate(ctx, out_buffer, &len, in_buffer, AES_BLOCK_SIZE);
        outfile.write(reinterpret_cast<char *>(out_buffer), len);
    }
