...
int Huffman::compress(const std::string &file, const std::string &targetFile)
{
	if (ScanCharacter(file) != 0)
		return -1;		 // 扫描源文件种字符的种类及个数
	...
	ifstream in(file, ios::binary); // 第二遍扫描源文件
	if (!in)
	{
		cout << "打开源文件失败" << endl
			 << endl;
...
void Huffman::CreateHuffmanTree()
{
	...
	while (mark < 2 * H_number)
	{
		...
		mark++;
		x = y = 0;
		min = INT32_MAX, secmin = 0;
	}
	HuffmanTree[2 * H_number - 1].p = 0;
}

// 扫描原文件中字符的种类及频度的函数
int Huffman::ScanCharacter(string file)
{
	ifstream in(file, ios::binary); // 以二进制的格式打开源文件
	if (!in)						// 检查指定的源文件是否存在
	{
		cout << "没有发现该文件.." << endl
			 << endl;
...
int Huffman::decompress(const std::string &file, const std::string &targetFile)
{
